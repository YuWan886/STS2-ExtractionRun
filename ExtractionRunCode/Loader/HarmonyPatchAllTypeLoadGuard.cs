...
///     type cannot be loaded (e.g. a patch references a game API the current
///     host no longer has).
/// </summary>
internal static class HarmonyPatchAllTypeLoadGuard
{
    private const string HarmonyId = "ExtractionRun.Loader.HarmonyPatchAllTypeLoadGuard";
    ...
    {
        (_warn ??= Console.Error.WriteLine)("[ExtractionRun.Loader.HarmonyPatchAllTypeLoadGuard] " + message);
    }
}
