<#
.SYNOPSIS
  Regenerates extractionrun-variants.manifest from the lib/<version>/ folders present under the
  mod root.

.PARAMETER ModRoot
  The mod install folder (the one containing <modId>.json / <modId>.pck).

.PARAMETER VariantAssembly
  The content variant assembly file name (default 'ExtractionRun.Content.dll').

.PARAMETER ManifestName
  The variant manifest file name (default 'extractionrun-variants.manifest').
#>
param(
    [Parameter(Mandatory = $true)][string]$ModRoot,
    [Parameter(Mandatory = $false)][string]$VariantAssembly = 'ExtractionRun.Content.dll',
    [Parameter(Mandatory = $false)][string]$ManifestName = 'extractionrun-variants.manifest'
)
...
$manifestPath = Join-Path $ModRoot $ManifestName
