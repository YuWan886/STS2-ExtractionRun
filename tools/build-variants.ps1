<#
.SYNOPSIS
  Builds the multi-version ExtractionRun bundle: one content variant per sts2.dll snapshot
  under -ApiRoot, plus a loader compiled against the OLDEST snapshot, assembled into the
  game's mods/ExtractionRun folder.

.PARAMETER ApiRoot
  Directory containing one sub-folder per game version, each holding sts2.dll
  (and optionally sts2.xml). Default: F:\sts2-mod\sts2-versions.
...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File tools/build-variants.ps1

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File tools/build-variants.ps1 -Versions 0.107.1
#>
param(
...
$modId = 'ExtractionRun'
$modCodeDir = "$modId" + 'Code'
$contentProject = Join-Path $repoRoot "$modId.csproj"
$loaderProject = Join-Path $repoRoot "$modCodeDir\Loader\$modCodeDir.Loader.csproj"
...
$modRoot = Join-Path $ModsPath $modId
...
try {
    ...
    & dotnet build $loaderProject -c $Configuration "/p:Sts2DataDir=$oldestDataDir"
    ...
    if (Test-Path (Join-Path $modRoot 'lib')) {
        Get-ChildItem (Join-Path $modRoot 'lib') | ForEach-Object {
            $dll = Join-Path $_.FullName $contentAssembly
